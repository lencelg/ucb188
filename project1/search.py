...
def depthFirstSearch(problem: SearchProblem) -> List[Directions]:
    ...
    while not frontier.isEmpty():
        ...
        if problem.isGoalState(state):
            return path
        for new_state, action,  _ in reversed(problem.getSuccessors(state)):
...
def breadthFirstSearch(problem: SearchProblem) -> List[Directions]:
    ...
    while not frontier.isEmpty():
        ...
        if problem.isGoalState(state):
            return path
        for new_state, action,  _ in problem.getSuccessors(state):
                new_path = path + [action]
                frontier.push((new_state, new_path))
...
def aStarSearch(problem: SearchProblem, heuristic=nullHeuristic) -> List[Directions]:
    ...
    record = {}
    record[current_state] = 0
    while not frontier.isEmpty():
        state, path, total_cost = frontier.pop()
        if record.get(state, float('inf')) < total_cost:
             continue
        if problem.isGoalState(state):
            return path
        for new_state, action,  cost in problem.getSuccessors(state):
            new_cost = total_cost + cost
            if new_cost < record.get(new_state, float('inf')):
                record[new_state] = new_cost
                new_path = path + [action]
                frontier.push((new_state, new_path, new_cost), heuristic(new_state, problem) + new_cost)
